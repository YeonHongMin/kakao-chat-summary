...
│   ├── chat_processor.py      # ChatProcessor 클래스 (추론 내용 제거)
│   ├── detail_prompt.py       # 상세 분석 프롬프트 + HTML 템플릿 + LLM API (v2.8.0)
...
│ 메뉴바                                                   │
│ ├─ 파일: 채팅방 추가, 채팅방 삭제, 종료                   │
│ ├─ 도구: 지금 동기화, LLM 요약 생성, 전체 채팅방 요약/URL/ │
│ │       상세분석, 백업/복원, DB 복구, 채팅방 복구, 설정     │
│ └─ 도움말: 정보                                          │
├──────────────┬──────────────────────────────────────────┤
│              │  QTabWidget (4개 탭)                      │
│  채팅방 목록  │  ├─ 📊 대시보드                           │
│  (QListWidget)│  ├─ 📅 날짜별 요약 (기본/상세 토글)       │
│              │  ├─ 🔗 URL 정보 (동기화/복구 버튼 포함)    │
│              │  └─ 🔧 기타 (통계 갱신 등)                │
│              │                                          │
│ [➕ 채팅방]   │                                          │
...
| `SummaryOptionsDialog` | LLM 요약 옵션 선택 (단일 채팅방) |
| `AllRoomsSummaryOptionsDialog` | 전체 채팅방 LLM 요약 옵션 (v2.7.0) |
...
| `SummaryGeneratorWorker` | LLM 요약 생성 (단일 채팅방) |
| `AllRoomsSummaryWorker` | 전체 채팅방 LLM 요약 생성 (v2.7.0) |
| `AllRoomsUrlSyncWorker` | 전체 채팅방 URL 동기화 (v2.7.0) |
| `DetailSummaryWorker` | 단일 날짜 상세 분석 HTML 생성 (v2.8.0) |
| `DetailBatchWorker` | 채팅방 내 여러 날짜 상세 분석 일괄 생성 (v2.8.0) |
| `AllRoomsDetailWorker` | 전체 채팅방 상세 분석 일괄 생성 (v2.8.0) |
...
### 2. LLM 요약 생성
- **지원 LLM**: Z.AI GLM (glm-4.5), OpenAI GPT-4o-mini, MiniMax (M2.7), Perplexity (sonar), DeepSeek 계열(qwen-or, qwen-kilo)
  - *제약사항*: DeepSeek 모델은 32K 컨텍스트 한계로 인해 데이터량이 많을 경우 오버플로우가 발생합니다. 따라서 `max_input_chars` 40,000자 / `max_tokens` 8000의 안전 가드가 적용되어 있습니다. 대화량이 방대한 방의 경우 입력이 잘릴 수 있으므로 128K~200K의 허용량을 가진 GLM이나 MiniMax 사용을 권장합니다.
- **요약 옵션**: 요약 필요한 날짜만 (기본), 오늘, 어제~오늘, 엇그제~오늘, 전체 일자 + 이미 요약된 날짜 건너뛰기 체크박스
- **상세 분석 함께 생성**: 기본 요약 완료 후 상세 분석 HTML 자동 생성 옵션 (기본 체크)
...
### 3. 상세 분석 HTML 생성 (v2.8.0)
- **별도 프롬프트** (`detail_prompt.py`): 토픽별 심층 분석, URL 모음, 감정/온도 분석, 핵심 시사점
- **다크 테마 HTML**: `data/detail_summary/`에 저장, 브라우저에서 열기 지원
- **생성 방식 3가지**:
  - 날짜 탭에서 단일 날짜 상세 생성 (`DetailSummaryWorker`)
  - 날짜 탭에서 일괄 상세 생성 (`DetailBatchWorker`)
  - 도구 메뉴에서 전체 채팅방 상세 분석 (`AllRoomsDetailWorker`, Ctrl+Shift+D)
...
- **기본 요약 / 상세 분석 토글 뷰** (v2.8.0)
  - 기본 뷰: 요약 마크다운 렌더링 + [📝 요약 생성] 버튼
  - 상세 뷰: 상세 분석 HTML 렌더링 + [🔍 상세 생성] [📂 브라우저] [🔍 일괄 상세] 버튼

### 6. URL 정보 탭
- **3개 섹션**: 최근 3일 (50개 제한), 최근 1주 (무제한), 전체 (무제한)
- **URL 상세 정보** (v2.8.2): 각 URL에 제목(@공유자), 내용, 시사점, 활용을 구조적으로 표시
- **멀티라인 URL 파싱** (v2.8.2): 요약의 새 URL 포맷(내용/시사점/활용)을 추출, 기존 포맷도 호환
...
- `data/detail_summary/`: 상세 분석 HTML (v2.8.0)
...
├── 지금 동기화 (Ctrl+R)
├── LLM 요약 생성 (Ctrl+G)
├── 🌐 전체 채팅방 LLM 요약 생성 (Ctrl+Shift+G)  ← v2.7.0
├── 🌐 전체 채팅방 URL 동기화 (Ctrl+Shift+U)      ← v2.7.0
├── 🌐 전체 채팅방 상세 분석 생성 (Ctrl+Shift+D)  ← v2.8.0
├── ─────────────
