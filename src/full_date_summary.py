"""
date_summarizer.py - 날짜별 요약 메인 모듈

카카오톡 대화 파일을 날짜별로 파싱하고,
각 날짜의 대화를 요약하여 하나의 통합 리포트를 생성합니다.

사용법:
    python date_summarizer.py <filepath>              # 단일 파일 (기본 LLM)
    python date_summarizer.py <directory>             # 디렉터리 일괄 처리
    python date_summarizer.py --llm chatgpt <file>    # ChatGPT 사용
    python date_summarizer.py                         # 대화형 모드
"""
...
class DateSummarizer:
    ...
    def _initialize_output_file(self, total_days: int):
        with open(self.output_file, 'w', encoding='utf-8') as f:
            f.write(f"# 📚 카카오톡 대화 요약 리포트\n")
            f.write(f"- **원본 파일**: {self.filepath.name}\n")
            f.write(f"- **총 대화 일수**: {total_days}일\n")
    ...
    def _extract_all_urls(self):
        ...
        try:
            ...
            if url_dict:
                ...
                print(f"    ✅ {len(url_dict)}개 URL 추출 -> {url_filename}")
            else:
                print("    ℹ️  추출된 URL이 없습니다.")
...
class BatchProcessor:
    ...
    def run(self):
        ...
        for idx, filepath in enumerate(target_files, 1):
            ...
            try:
                ...
                results.append((filepath.name, "✅ 성공" if success else "⚠️ 스킵"))
            except Exception as e:
                logger.exception(f"Failed to process {filepath.name}")
                results.append((filepath.name, f"❌ 실패"))
    ...
    def _print_final_results(self, results: List[tuple]):
        ...
        success_count = sum(1 for _, status in results if "성공" in status)
        
        for filename, status in results:
            print(f"  {status} {filename}")
        
        print("-"*60)
        print(f"총 {len(results)}개 | ✅ 성공: {success_count} | 기타: {len(results) - success_count}")
...
def main():
    ...
    if not target:
        print("Usage:")
        print("  python date_summarizer.py <file>               # 단일 파일")
        print("  python date_summarizer.py <directory>          # 디렉터리")
        print("  python date_summarizer.py --llm chatgpt <file> # LLM 지정\n")
