...
### 7.2 파일-DB 동기화 규칙 (v2.4.0 업데이트)

1. **원본 파일이 진실의 원천**: 파일 내용이 DB와 다르면 파일 우선
2. **파일 크기 기반 요약 무효화**: 재업로드 시 저장 전/후 파일 크기 비교 → 변경된 날짜만 요약 무효화
3. **데이터 감소 방지 (Safety Check)**: 새 파일의 메시지 수가 기존 파일보다 적으면 덮어쓰지 않음 (Data Loss 방지)
4. **요약 백업 정책**: 요약 무효화 시 기존 파일을 `.bak`로 백업 (영구 삭제 방지)
5. **중복 메시지 처리**: 동일한 (날짜, 시간, 발신자, 내용) → 스킵
