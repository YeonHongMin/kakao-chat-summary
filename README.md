...
Cursor와 분리된 백그라운드 실행입니다. 오류는 `logs\startup_stderr.txt`를 확인하세요.
...
- 🔗 **URL 탭**: 백그라운드 Worker 로드, 섹션당 50개 표시 제한, 로드 중 방 전환 가능
- ⏹️ **상세 분석 취소**: LLM API 대기 중 즉시 중단 (취소 후 미완료 날짜는 저장하지 않음)
- 🛠️ **start_background.ps1**: 중복 프로세스 정리, 백그라운드 stderr 로그
