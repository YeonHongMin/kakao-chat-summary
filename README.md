...
- ⏰ **엇그제~오늘 요약**: 2일 전부터 오늘까지의 대화만 요약하는 스크립트
...
### 2. 전체 날짜 요약 (full_date_summary.py / simple_date_summary.py)

모든 날짜의 대화를 순차적으로 요약합니다. **단일 파일 또는 디렉터리 지원.**

```bash
# 상세 요약 (full) - 기본 LLM (GLM)
python src/full_date_summary.py data/채팅방.txt
python src/full_date_summary.py --llm minimax data/

# 간결 요약 (simple, 음슴체)
python src/simple_date_summary.py data/채팅방.txt
python src/simple_date_summary.py --llm minimax data/
...
**출력 파일**:
- `채팅방_summaries.md` - 날짜별 통합 요약 리포트 (Full)
- `채팅방_simple_summaries.md` - 날짜별 통합 요약 리포트 (Simple)
...
### 4. 2일(엇그제)~오늘 요약 (full_2days_summary.py / simple_2days_summary.py)

**2일 전(엇그제)부터 오늘(현재 시점)까지**의 대화만 요약합니다.

```bash
# 상세 요약 (full)
python src/full_2days_summary.py data/채팅방.txt
python src/full_2days_summary.py --llm minimax data/

# 간결 요약 (simple, 음슴체)
python src/simple_2days_summary.py data/채팅방.txt
python src/simple_2days_summary.py --llm minimax data/
```

**출력 파일**:
- `채팅방_full_2days_summary.md` - 엇그제~오늘 상세 요약
- `채팅방_simple_2days_summary.md` - 엇그제~오늘 간결 요약

### 5. 오늘만 요약 (full_today_summary.py / simple_today_summary.py)
...
│   ├── full_date_summary.py         # 전체 날짜 상세 요약 (메인)
│   ├── simple_date_summary.py       # 전체 날짜 간결 요약 (음슴체)
