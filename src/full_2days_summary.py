"""
full_2days_summary.py - 2일 전(엇그제)부터 오늘까지 대화 요약 모듈

파일 내에 존재하는 엇그제부터 오늘까지의 대화를 날짜별로 구분하여 요약을 생성합니다.

사용법:
    python full_2days_summary.py <filepath>              # 단일 파일
    python full_2days_summary.py <directory>             # 디렉터리 일괄
    python full_2days_summary.py --llm chatgpt <file>    # LLM 지정
    python full_2days_summary.py                         # 대화형 모드
"""
...
class Full2DaysSummarizer:
    """2일 전(엇그제)부터 오늘까지의 대화를 요약하는 클래스."""
    ...
    def __init__(self, filepath: Path, provider: Optional[str] = None):
        ...
        # 출력 파일명: _full_2days_summary.md
        self.output_file = filepath.parent / f"{filepath.stem}_full_2days_summary.md"

    def run(self) -> bool:
        """2일 전부터 오늘까지 요약 처리를 실행."""
        ...
        if not target_dates:
            print(f"   ⚠️  엇그제부터 오늘까지의 대화가 없습니다.")
            return False
        ...
        for date in target_dates:
            ...
            msg_count = sum(1 for line in messages if ts_pattern.search(line))
...
class BatchProcessor:
    ...
    def run(self):
        ...
        print("="*60)
        print("📅 2일 전(엇그제)부터 오늘까지 대화 일괄 요약")
        print("="*60)
        ...
        for filepath in target_files:
            summarizer = Full2DaysSummarizer(filepath, self.provider)
...
def main():
    ...
    print("="*50)
    print(f"📅 2일 전(엇그제)부터 오늘까지 대화 요약기")
    print("="*50)
    
    # 명령줄 인자 없으면 대화형 모드
    if not target:
        print("Usage:")
        print("  python full_2days_summary.py <file>")
        print("  python full_2days_summary.py <directory>")
        print("  python full_2days_summary.py --llm chatgpt <file>\n")
        ...
        if data_dir.exists():
            ...
            if txt_files:
                ...
                if choice.upper() == 'A':
                    ...
                    prompt_api_key()
                    summarizer = Full2DaysSummarizer(target_file, selected_provider)
                    summarizer.run()
                    sys.exit(0)
        sys.exit(1)
    ...
    if target_path.is_dir():
        ...
        summarizer = Full2DaysSummarizer(target_path, provider)
        summarizer.run()
