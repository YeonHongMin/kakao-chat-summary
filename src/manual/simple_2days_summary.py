"""
simple_2days_summary.py - 2일 전(엇그제)부터 오늘까지 대화 간결 요약 모듈

음슴체/단답형으로 짧고 간결한 요약을 생성합니다.
파일 내에 존재하는 엇그제부터 오늘까지의 대화를 날짜별로 구분하여 요약합니다.

사용법:
    python simple_2days_summary.py <filepath>              # 단일 파일
    python simple_2days_summary.py <directory>             # 디렉터리 일괄
    python simple_2days_summary.py --llm chatgpt <file>    # LLM 지정
    python simple_2days_summary.py                         # 대화형 모드
"""
...
class SimpleConfig:
    ...
    def _setup_logging(self) -> None:
        ...
        log_filename = f"simple_2days_summarizer_{datetime.now().strftime('%Y%m%d')}.log"
        ...
        logger = logging.getLogger("Simple2DaysSummarizer")
    ...
    def logger(self) -> logging.Logger:
        return logging.getLogger("Simple2DaysSummarizer")
...
# ============================================================
# 메인 요약 클래스 (2일 전부터 오늘까지)
# ============================================================

class Simple2DaysSummarizer:
    """2일 전(엇그제)부터 오늘까지 대화 간결 요약 클래스"""
    
    def __init__(self, filepath: Path, provider: Optional[str] = None):
        self.filepath = filepath
        self.parser = SimpleParser()
        self.llm = SimpleLLMClient()
        # 출력 파일명: _simple_2days_summary.md
        self.output_file = filepath.parent / f"{filepath.stem}_simple_2days_summary.md"

    def run(self) -> bool:
        """2일 전부터 오늘까지 요약 처리 실행"""
        ...
        # 날짜 필터링: 2일 전(엇그제)부터 오늘까지
        today_date = datetime.now().strftime("%Y-%m-%d")
        yesterday_date = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d")
        day_before_yesterday = (datetime.now() - timedelta(days=2)).strftime("%Y-%m-%d")

        all_dates = sorted(parse_result.messages_by_date.keys())
        target_dates = [d for d in all_dates if d >= day_before_yesterday]  # 엇그제 이후의 날짜만

        print(f"   📅 전체 날짜: {len(all_dates)}일")
        print(f"   🎯 대상 날짜 (엇그제~오늘): {len(target_dates)}일 ({', '.join(target_dates)})")
        
        if not target_dates:
            print(f"   ⚠️  엇그제부터 오늘까지 대화 없음")
            return False
        ...
        for date in target_dates:
            ...
            import re
            # 단순 라인 수가 아니라 실제 메시지(말풍선) 개수 카운트
            # 패턴: [닉네임] [오전/오후 00:00]
            ts_pattern = re.compile(r'\[.*?\] \[(오전|오후) \d{1,2}:\d{2}\]')
            count_val = sum(1 for line in messages if ts_pattern.search(line))
            
            # 파싱된 메시지가 없으면 0이 될 수 있으므로, 최소 1개 이상 라인이 있으면 원래 방식과 병행 고려?
            # 일단 정확하게 타임스탬프 기준을 사용합니다.
            if count_val > 0:
                msg_count = count_val
...
class SimpleBatchProcessor:
    ...
    def run(self):
        ...
        print("=" * 50)
        print(f"📅 2일 전(엇그제)부터 오늘까지 간결 요약")
        print("=" * 50)
        ...
        results = []
        for f in files:
            summarizer = Simple2DaysSummarizer(f, self.provider)
...
def main():
    ...
    print("=" * 50)
    print(f"📅 2일 전(엇그제)부터 오늘까지 간결 요약기")
    print("=" * 50)
    
    if not target:
        print("Usage: python simple_2days_summary.py <file|dir>")
        print("       python simple_2days_summary.py --llm chatgpt <file>\n")
        ...
        if data_dir.exists():
            ...
            if files:
                ...
                if choice.upper() == 'A':
                    ...
                    prompt_api_key()
                    Simple2DaysSummarizer(files[int(choice)-1], selected).run()
                    sys.exit(0)
        sys.exit(1)
    ...
    if target_path.is_dir():
        ...
        Simple2DaysSummarizer(target_path, provider).run()
