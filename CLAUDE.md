...
| `SettingsDialog` | 설정 (LLM 제공자·API 키 → `.env.local` 저장) |
| (QMessageBox) | 각종 알림 |
...
save_provider_to_env(provider)      # LLM_PROVIDER → .env.local
save_api_key_to_env(api_key, provider)  # 빈 값이면 파일 미갱신
```
...
도구 → **설정** 다이얼로그에서도 `LLM_PROVIDER`와 API 키를 저장할 수 있습니다 (v2.9.8).
...
2. [ ] 새 메시지 개수 표시
3. [x] 설정 다이얼로그에서 API 키 입력 (v2.9.8 완료)
4. [ ] 요약 품질 평가 기능
5. [ ] 테스트 코드 작성 (pytest)
...
### v2.9.7 - Windows cp949 콘솔 인코딩 호환 (2026-06-08)

- 🐛 **파일 업로드 시 `UnicodeEncodeError: 'cp949' codec can't encode character 'ℹ'` 수정**
  - 증상: Windows에서 `python.exe src/app.py`로 띄운 뒤 파일 업로드 시 워커가 `❌ 오류: 'cp949' codec ...`로 종료. `data/original/` 저장 후 `print(f"ℹ️ ... 과거 날짜 원본 파일 보호")` 호출 지점에서 Windows 콘솔 기본 인코딩(cp949)이 ℹ️(U+2139)를 인코딩하지 못해 발생.
  - 수정: `src/app.py` 진입 시점에 `sys.stdout` / `sys.stderr`를 `utf-8 + errors="replace"`로 재설정. `reconfigure` 미지원·`None` 스트림(pythonw, 리다이렉트)에서는 안전하게 스킵.
  - 크로스 플랫폼: macOS/Linux는 stdout이 이미 UTF-8이라 no-op. Windows에서만 실질적으로 동작.

### v2.9.8 - 설정 다이얼로그·MiMo·컨텍스트 상한 정비 (2026-07-04)
- ⚙️ **설정 다이얼로그**: `save_provider_to_env`, `save_api_key_to_env` — LLM 제공자·API 키를 `.env.local`에 영구 저장
- 🛡️ **빈 키 보호**: API 키 미입력 시 기존 `.env.local` 값 유지
- 🆕 **Xiaomi MiMo**: `mimo-v2.5-pro`, `detail_prompt` MiMo API 호환 (`max_completion_tokens`, `thinking: disabled`)
- 📏 **입력 컨텍스트 상한**: `_input_chars_from_context()` — GLM-5.2 1450848 / OpenAI 167424 / MiniMax 718848 / Perplexity 168000 / MiMo 1450848 chars

### v2.9.6 - 성능 튜닝 및 UI 반응성 개선 (2026-05-25)

- **지연 로딩(Lazy Loading)**: 채팅방 클릭 시 당장 보지 않는 'URL 정보', '날짜별 요약' 데이터 로딩을 탭 활성화 시점까지 지연
- **UI 프리징 해결**: 무거운 HTML 및 JSON 파싱이 메인 스레드를 멈추게 하던 병목 현상 제거
- **QTimer 이벤트 분산**: 방 클릭 즉시 하이라이트가 적용되도록 `QTimer.singleShot` 적용

---

### v2.9.5 - 토큰·로그·예제 env 정비 (2026-05-05)

- **MiniMax**: `MINIMAX_MAX_TOKENS` 기본 32768; `finish_reason=length` 이고 본문에 `<h2>` 없을 때 최소 헤더 삽입 후 저장 가능하게 처리
- **GLM**: `ZAI_MAX_TOKENS` 기본 8192 → 32768
- **로그**: `call_detail_llm` 모든 주요 로그에 `[room_name | date_str]` 접두사
- **env.local.example**: 누락 복구, `ZAI_MAX_TOKENS` / `ZAI_MAX_INPUT_CHARS` 등 문서화, 실수로 넣은 활성 값 제거
- **styles.py**: `QTextBrowser` 선택 배경/글자색 (카카오 노랑 계열)
- 상세: `CHANGELOG.md`

---

### v2.9.4 - 기본 LLM MiniMax 및 설정·LLM UI 정비 (2026-05-04)

- **기본 LLM**: `Config.DEFAULT_PROVIDER = "minimax"` — 상세 분석·워커 기본 인자·`call_detail_llm` 기본 provider와 일치
- **`LLM_PROVIDER`**: 비어 있거나 `LLM_PROVIDERS`에 없으면 `DEFAULT_PROVIDER` 사용 (빈 문자열로 콤보 인덱스 0(GLM) 고정되던 문제 수정)
- **다이얼로그**: 전체 채팅방 상세·날짜별·일괄·Ctrl+G 옵션에서 콤보 기본 선택 시 `config.DEFAULT_PROVIDER` 폴백
- **설정 다이얼로그**: `LLM_PROVIDERS` 기반 동적 목록, 확인 시 `config.set_provider()` (세션 내; 재기동 시 `.env.local`의 `LLM_PROVIDER` 우선)
- **`env.local.example`**: MiniMax 기본 안내·`LLM_PROVIDER=minimax` 예시 주석
- **버전 문자열**: `app.py` `setApplicationVersion`, 도움말 About → v2.9.4
