# 03. User Flow

## 1. GUI 애플리케이션 흐름

### 1.1 애플리케이션 시작

```
┌─────────────────────────────────────────────────────────────┐
│                    앱 실행 (python src/app.py)              │
└──────────────────────────┬──────────────────────────────────┘
                           │
                           ▼
              ┌────────────────────────┐
              │  .env.local 로드       │
              │  (API 키·LLM_PROVIDER) │
              └───────────┬────────────┘
                          │
                          ▼
              ┌────────────────────────┐
              │  SQLite DB 연결        │
              │  (data/db/chat_history.db)│
              └───────────┬────────────┘
                          │
                          ▼
              ┌────────────────────────┐
              │  메인 윈도우 표시      │
              │  - 채팅방 목록 로드    │
              │  - 대시보드 탭 활성화  │
              └────────────────────────┘
```

---

### 1.1.1 설정 (LLM·API 키) — v2.9.8

```
[도구 → 설정] 클릭
        │
        ▼
┌────────────────────────┐
│  ⚙️ 설정 다이얼로그    │
│  ──────────────────── │
│  🤖 LLM 제공자: [▼]    │
│  API 키: [••••••••]    │
│  ──────────────────── │
│  [확인]  [취소]        │
└───────────┬────────────┘
            │ 확인
            ▼
┌────────────────────────┐
│  .env.local 갱신       │
│  - LLM_PROVIDER        │
│  - {PROVIDER}_API_KEY  │
│    (키 입력 시만)      │
└────────────────────────┘
```

- 제공자 변경 시 해당 키를 입력란에 불러옴 (Ollama는 키 불필요)
- API 키를 비운 채 확인하면 기존 `.env.local` 키는 유지

---

### 1.2 채팅방 관리 흐름

```
┌─────────────────────────────────────────────────────────────┐
│                       메인 화면                             │
├─────────────────────────────────────────────────────────────┤
│  ┌───────────────┐  ┌─────────────────────────────────────┐ │
│  │  채팅방 목록  │  │          콘텐츠 영역               │ │
│  │  ──────────── │  │  ┌─────────────────────────────────┐│ │
│  │  📁 개발팀│  │  │  📊 대시보드 | 📅 날짜별 | 🔗 URL | 🔧 기타 ││ │
│  │  📁 스터디방  │  │  │─────────────────────────────────││ │
│  │               │  │  │                                 ││ │
│  │  [➕ 채팅방]  │  │  │    통계 / 요약 / URL 정보      ││ │
│  │  [📤 업로드]  │  │  │                                 ││ │
│  └───────────────┘  │  └─────────────────────────────────┘│ │
│                     └─────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
```

---

## 2. 주요 사용 시나리오

### 시나리오 A: 새 채팅방 추가 및 파일 업로드

```
[➕ 채팅방 만들기] 클릭
        │
        ▼
┌────────────────────────┐
│  채팅방 이름 입력      │
│  예: "개발팀"      │
└───────────┬────────────┘
            │
            ▼
┌────────────────────────┐
│  채팅방 생성 완료      │
│  - DB에 레코드 생성    │
│  - 디렉터리 생성       │
│    data/original/개발팀/
│    data/summary/개발팀/
└───────────┬────────────┘
            │
            ▼
[📤 파일 업로드] 클릭
        │
        ▼
┌────────────────────────┐
│  채팅방 선택           │
│  파일 선택 (.txt)      │
└───────────┬────────────┘
            │
            ▼
┌────────────────────────┐
│  백그라운드 처리       │
│  - 파일 파싱           │
│  - 날짜별 그룹화       │
│  - DB 저장 (중복 체크) │
│  - 파일 저장           │
│  - 기존 요약 무효화    │
└───────────┬────────────┘
            │
            ▼
┌────────────────────────┐
│  업로드 완료 알림      │
│  "2024-01-20 ~ 01-25   │
│   총 1,234개 메시지"   │
└────────────────────────┘
```

---

### 시나리오 B: 요약 생성

```
[🔄 요약 생성] 클릭
        │
        ▼
┌────────────────────────┐
│  요약 옵션 다이얼로그  │
│  ──────────────────── │
│  📅 날짜 범위:         │
│    ● 요약 필요한 날짜만│
│    ○ 오늘             │
│    ○ 어제~오늘        │
│    ○ 엇그제~오늘      │
│    ○ 전체 일자        │
│  ──────────────────── │
│  🤖 LLM 선택:          │
│    [Z.AI GLM ▼]        │
│    API 상태: ✅ 설정됨 │
│  ──────────────────── │
│  ☑ 이미 완료된 날짜 건너뛰기│
│  ──────────────────── │
│  📊 요약 현황:         │
│    전체: 30일 / 완료: 25일│
│    신규: 3일 / 업데이트: 2일│
└───────────┬────────────┘
            │
            ▼
상태바에 비모달 프로그레스 위젯 표시 (메인 윈도우 조작 가능)
┌────────────────────────────────────────────────────────────────┐
│  ⏳ 요약 중...   [🤖 [채팅방] Z.AI GLM 요약 중... ▓▓▓░░ ❌]  (15:31:10) │
└───────────┬────────────────────────────────────────────────────┘
            │
            ▼ (완료 시 위젯 제거)
┌────────────────────────────────────────────────────────────────┐
│  ✅ [채팅방] 요약 완료                                 (15:32:45) │
└────────────────────────────────────────────────────────────────┘
```

---

### 시나리오 C: 날짜별 상세 분석 조회

```
[📅 날짜별 상세 분석] 탭 클릭
        │
        ▼
┌────────────────────────────────────────────────────┐
│  ◀ 이전    📅 2024년 01월 23일    📆    다음 ▶   │
├────────────────────────────────────────────────────┤
│  📊 123개 메시지 / 요약 있음                       │
├────────────────────────────────────────────────────┤
│                                                    │
│  ## 🌟 3줄 요약                                    │
│  1. 새 프로젝트 킥오프 회의 진행                   │
│  2. 기술 스택 논의 (Python, FastAPI)               │
│  3. 다음 주 스프린트 계획 수립                     │
│                                                    │
│  ## ❓ Q&A 및 해결된 문제                          │
│  - Q: FastAPI vs Django?                           │
│    A: 빠른 개발에는 FastAPI 권장                   │
│  ...                                               │
│                                                    │
└────────────────────────────────────────────────────┘
```

---

### 시나리오 D: URL 정보 조회

```
[🔗 URL 정보] 탭 클릭
        │
        ▼
┌────────────────────────────────────────────────────┐
│  🔗 공유된 URL 목록      0개 URL  [🔄 동기화] [📂 파일 복구] │
├────────────────────────────────────────────────────┤
│                                                    │
│  📊 총 415개 URL                                   │
│  ─────────────────────────────────────────────────│
│                                                    │
│  🔥 최근 3일 (25개)                                │
│  ─────────────────────────────────────────────────│
│    #1 https://example.com/today                   │
│        : 오늘 공유된 링크                          │
│                                                    │
│  📅 최근 1주 (98개)                                │
│  ─────────────────────────────────────────────────│
│    #1 https://example.com/weekly                  │
│        : 이번 주 공유된 링크                       │
│                                                    │
│  📚 전체 URL (415개)                               │
│  ─────────────────────────────────────────────────│
│    #1 https://fastapi.tiangolo.com                │
│        : FastAPI 공식 문서                         │
│    #2 https://github.com/example/project          │
│        : 프로젝트 레포 / 소스코드 저장소           │
│  ...                                               │
└────────────────────────────────────────────────────┘
```

---

### 시나리오 E: 백업/복원 (v2.9.11)

```
[도구] 메뉴 → [💾 전체 백업...] 또는 [💾 채팅방 백업...]
        │
        ▼
┌────────────────────────┐
│  백업 확인 다이얼로그  │
│  (백업 대상 표시)      │
│  [취소]     [백업]     │
└───────────┬────────────┘
            │
            ▼
┌────────────────────────────────────────┐
│  상태바: 💾 [전체] 복사 중... (n/N) ██ │
│  [❌ 취소]                             │
└───────────┬────────────────────────────┘
            │
            ▼
┌────────────────────────┐
│  백업 완료 알림        │
│  📁 data/backup/...    │
└────────────────────────┘

[도구] 메뉴 → [📂 전체 백업에서 복원...] 또는 [📂 채팅방 복원...]
        │  (백업/상세 분석/복구가 진행 중이면 차단)
        ▼
┌────────────────────────┐
│  백업 선택 다이얼로그  │
│  [ ] 20260204_120000   │
│  [ ] 20260203_153000   │
└───────────┬────────────┘
            │
            ▼
┌────────────────────────┐
│  채팅방 복원 시:       │
│  백업 내 채팅방 선택   │
│  (예: 스터디방)        │
│  전체 복원 시: DB 포함 │
│  (WAL/SHM 세트 교체)   │
└───────────┬────────────┘
            │
            ▼
┌────────────────────────┐
│  복원 완료 알림        │
│  (전체 복원 시 재시작 권장) │
└────────────────────────┘
```

---

### 시나리오 F: 파일↔DB 동기화

```
[도구] 메뉴 → [🔄 파일에서 DB 재구축...] 또는 [🔄 누락 채팅방 DB 추가...]
        │
        ▼
┌────────────────────────┐
│  확인 다이얼로그       │
│  (동기화 대상 표시)    │
│  [취소]     [실행]     │
└───────────┬────────────┘
            │
            ▼
┌────────────────────────┐
│  완료 알림             │
└────────────────────────┘

* 파일에서 DB 재구축: 파괴적 (기존 DB 삭제 후 파일에서 재생성)
* 누락 채팅방 DB 추가: 비파괴적 (파일에는 있지만 DB에 없는 채팅방만 추가)
```

---

### 시나리오 G: 전체 채팅방 LLM 요약 생성 (v2.7.0)

```
[도구] 메뉴 → [🌐 전체 채팅방 LLM 요약 생성] (Ctrl+Shift+G)
        │
        ▼
┌────────────────────────────────────┐
│  전체 채팅방 LLM 요약 생성 옵션   │
│  ──────────────────────────────── │
│  🏠 채팅방 5개 | 📅 총 120일 |   │
│  ✅ 완료 80일 | 🔄 요약필요 40일  │
│  ──────────────────────────────── │
│  📋 채팅방별 현황                  │
│    🔴 개발팀: 25/30일 (🔄 5일 필요) │
│    🟢 스터디방: 20/20일 완료      │
│    ...                             │
│  ──────────────────────────────── │
│  🤖 LLM 선택: [Z.AI GLM ▼] ✅    │
│  📅 범위: ● 요약 필요한 날짜만    │
│  ──────────────────────────────── │
│  ⚠️ 시간이 오래 걸릴 수 있습니다  │
│  [취소]  [🌐 전체 채팅방 요약 시작]│
└───────────┬────────────────────────┘
            │
            ▼
상태바에 진행 표시
┌──────────────────────────────────────────────────────────┐
│  ⏳ [1/5] 개발팀 - 2026-03-25 (3/5) [▓▓▓░░ ❌] (15:31) │
└───────────┬──────────────────────────────────────────────┘
            │
            ▼ (완료)
┌────────────────────────────────────┐
│  전체 채팅방 요약 완료             │
│  ✅ 전체 완료: 40일 요약           │
│  --- 채팅방별 결과 ---             │
│  ✅ 개발팀: 5일 완료               │
│  ✅ 스터디방: 0일 (요약 필요 없음)  │
│  ⏭️ 가족방: 데이터 없음            │
└────────────────────────────────────┘
```

---

### 시나리오 H: 전체 채팅방 URL 동기화 (v2.7.0)

```
[도구] 메뉴 → [🌐 전체 채팅방 URL 동기화] (Ctrl+Shift+U)
        │
        ▼
┌────────────────────────────────────┐
│  전체 채팅방 URL 동기화            │
│  다음 5개 채팅방의 요약에서        │
│  URL을 추출합니다:                 │
│    • 개발팀                        │
│    • 스터디방                      │
│    • 가족방                        │
│    ...                             │
│  [아니오]          [예]            │
└───────────┬────────────────────────┘
            │
            ▼
상태바에 진행 표시
┌──────────────────────────────────────────────────────────┐
│  ⏳ [2/5] 스터디방 URL 수집 중... [▓▓▓░░ ❌]    (15:31) │
└───────────┬──────────────────────────────────────────────┘
            │
            ▼ (완료)
┌────────────────────────────────────┐
│  전체 채팅방 URL 동기화 완료       │
│  ✅ 전체 완료: 415개 URL 수집      │
│  --- 채팅방별 결과 ---             │
│  ✅ 개발팀: 200개 URL              │
│  ✅ 스터디방: 150개 URL            │
│  ⏭️ 가족방: URL 없음               │
└────────────────────────────────────┘
```

---

## 3. CLI 사용 (수동 요약용, 레거시)

`src/manual/` 디렉터리의 CLI 스크립트는 GUI 앱과 별개로 동작합니다.
DB/FileStorage를 사용하지 않으며, 결과는 `output/` 디렉터리에 저장됩니다.

### Full 스크립트 (상세 요약)
src/ 모듈을 재사용합니다 (`full_config`, `parser`, `chat_processor`, `url_extractor`).

```bash
python src/manual/full_date_summary.py data/채팅방.txt
python src/manual/full_yesterday_summary.py --llm minimax data/
python src/manual/full_2days_summary.py data/채팅방.txt
python src/manual/full_today_summary.py --llm glm data/
```

### Simple 스크립트 (간결 요약, 음슴체)
자체 내장 구현(`SimpleConfig`, `SimpleParser`, `SimpleLLMClient`)으로 외부 모듈 의존 없이 단독 실행 가능합니다.

```bash
python src/manual/simple_date_summary.py data/채팅방.txt
python src/manual/simple_today_summary.py --llm glm data/
python src/manual/simple_2days_summary.py data/채팅방.txt
python src/manual/simple_yesterday_summary.py --llm minimax data/
```

### 실행 모드
```bash
# 단일 파일 지정
python src/manual/<스크립트>.py <파일.txt>

# 디렉터리 일괄 처리
python src/manual/<스크립트>.py <디렉터리>

# LLM 선택
python src/manual/<스크립트>.py --llm chatgpt <파일.txt>

# 인자 없이 실행 → 대화형 모드
python src/manual/<스크립트>.py
```

---

## 4. 상태바 표시

```
┌──────────────────────────────────────────────────────────────────────────┐
│  ✅ 준비                                                      (15:30:25) │
│  ⏳ 요약 중...  [🤖 [채팅방] GLM 요약 중... ▓▓▓░░ ❌]         (15:31:10) │
│  ✅ [채팅방] 요약 완료                                         (15:32:45) │
│  ❌ 요약 생성 실패: 채팅방                                     (15:32:45) │
└──────────────────────────────────────────────────────────────────────────┘

아이콘 의미:
  ✅ 성공/준비 완료
  ⏳ 작업 진행 중
  ❌ 실패
  ⚠️ 경고
  ℹ️ 정보
```
